...
- Infrastructure as Code: Use Terraform to provision VMs and output their details.
...
- Ansible for configuration consistency and idempotent setup.

- Docker Compose for containerized monitoring services.

- Separation of provisioning and configuration for maintainability.

- Reproducible and modular folder structure.
...
- You have access to the internet on the control node ( a Linux VM, e.g., Ubuntu).
- You can connect via SSH or already have a terminal open on the node.

---

## Setup VS Code (on your laptop) to control the project on VM (optional)
...
1. **Configure Variables:**  
   ...
   Update `vms.tf` to add additional VMs.
...
- Only required ports are exposed via NSG (22, 3000, 9090, 9093, 3001, 9100). All credentials are managed securely, and no secrets are hard-coded.
